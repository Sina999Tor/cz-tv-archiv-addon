...
function withYearScan(fetchRawPage, dateField, year) {
  return async (p) => {
    ...
    const years = raw
      ...
      .filter(y => !isNaN(y));
    // Zastavit scan smí jen CELÁ stránka, která je systematicky starší než
    // hledaný rok — ne jediná položka. TMDB primary_release_date je u CZ/SK
    // titulů nespolehlivé (remasterované klasiky, chybné datové záznamy...),
    // takže dřívější "stačí jeden nejstarší film v dávce" (Math.min) uměl
    // usekout scan hned po první dávce kvůli jediné vadné položce — a to
    // prakticky nezávisle na tom, jaký rok se hledal. Teď musí být VŠECHNY
    // datované položky na stránce starší, ať jeden outlier scan nezastaví.
    const allOld = years.length > 0 && years.every(y => y < Number(year) - 1);
    return { ...data, results, stop: allOld };
  };
}
