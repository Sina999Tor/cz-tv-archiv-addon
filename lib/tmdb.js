...
export async function discoverMovies(apiKey, { sort = 'Populární', genre = '', search = '', skip = 0, country = 'CZ + SK', year = '' }) {
  ...
  const rawPage = (p) => tmdbGet(apiKey, '/discover/movie', {
    ...
    page: p,
    // Vždy od nejnovějšího po nejstarší (manifest už nenabízí volbu řazení/roku).
  ...
  return collectPage({
    ...
    mapItem,
    getKey: (m) => m && m.id,
    progressKey: `movie-discover:${apiKey}:${stableKey({ genre, country })}`,
...
export async function discoverSeries(apiKey, { sort = 'Populární', genre = '', search = '', skip = 0, country = 'CZ + SK', year = '' }) {
  ...
  return collectPage({
    ...
    mapItem,
    getKey: (s) => s && s.id,
    progressKey: `series-discover:${apiKey}:${stableKey({ genre, country })}`,
    fetchTmdbPage: rawPage
  });
}
