...
export async function discoverMovies(apiKey, { genre = 'Populární', search = '', skip = 0, country = 'CZ + SK', year = '' }) {
  ...
  return collectPage({
    ...
    // v tom roce fakticky vyšly. Proto se použije release_date.gte/lte + region (najde
    // i regionální vydání) a výsledek se ještě dofiltruje na klientu podle skutečného
    // roku vydání — stejný postup jako u /search/movie výše.
    fetchTmdbPage: async (p) => {
