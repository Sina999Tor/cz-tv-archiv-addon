...
// --- KATALOG: FILMY ---
export async function discoverMovies(apiKey, { search = '', skip = 0, country = 'CZ + SK', year = '' }) {
  const mapItem = async (m) => {
    ...
    if (releaseYear && releaseYear > CURRENT_YEAR) return null; // ještě nevyšlo — pojistka nad rámec TMDB filtru
    let imdbId = null;
    try {
  ...
  return collectPage({
    ...
    mapItem,
    getKey: (m) => m && m.id,
    progressKey: `movie-discover:${apiKey}:${stableKey({ country, year })}`,
    fetchTmdbPage: year ? withYearScan(rawPage, 'release_date', year) : rawPage
  });
}

// --- KATALOG: SERIÁLY ---
export async function discoverSeries(apiKey, { search = '', skip = 0, country = 'CZ + SK', year = '' }) {
  const mapItem = async (s) => {
    if (!s || !s.id || !s.name) return null;
    const airYear = Number((s.first_air_date || '').slice(0, 4));
    if (airYear && airYear > CURRENT_YEAR) return null; // ještě neproběhla premiéra — pojistka nad rámec TMDB filtru
    let imdbId = null;
    try {
  ...
  return collectPage({
    ...
    mapItem,
    getKey: (s) => s && s.id,
    progressKey: `series-discover:${apiKey}:${stableKey({ country, year })}`,
