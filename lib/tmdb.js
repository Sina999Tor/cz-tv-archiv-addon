...
export async function discoverMovies(apiKey, { genre = 'Populární', search = '', skip = 0, country = 'CZ + SK', year = '' }) {
  ...
  return collectPage({
    ...
    mapItem,
    getKey: (m) => m && m.id,
    // primary_release_date.gte/lte matchuje jen "primární" (typicky US) vydání filmu.
    // Spousta CZ/SK titulů primární vydání buď nemá vyplněné, nebo je vedené jinak než
    // český/slovenský release, takže při filtru na konkrétní rok spadnou mimo, i když
    // v tom roce fakticky vyšly. Proto se použije release_date.gte/lte + region, což
    // TMDB vyhodnotí podle skutečného regionálního (CZ) vydání.
    // POZOR: výsledek se dál NEFILTRUJE podle m.release_date — to pole je pořád jen
    // to "primární" (globální) datum, takže dofiltrování jím by zase vyhodilo přesně
    // ty tituly, kvůli kterým se region.gte/lte vůbec zavádí (region filtr TMDB udělá
    // správně na serveru, m.release_date v odpovědi to jen nereflektuje).
    fetchTmdbPage: (p) => tmdbGet(apiKey, '/discover/movie', {
      with_original_language: languageParam(country),
      page: p,
      ...(year ? { 'release_date.gte': `${year}-01-01`, 'release_date.lte': `${year}-12-31`, region: 'CZ' } : {}),
      ...sortParam(genre)
    })
  });
}
